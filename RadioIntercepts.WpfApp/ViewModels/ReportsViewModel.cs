...
namespace RadioIntercepts.WpfApp.ViewModels
{
    public class ReportViewModel : INotifyPropertyChanged
    {
        ...
        private readonly Services.IAlertService _alertService;
        ...
        public ReportViewModel(Services.IReportService reportService, Services.IAlertService alertService)
        {
    ...
    {
        private readonly Core.Models.ReportTemplate _template;
        private List<ReportParameterViewModel> _parameters = new();

        public ReportTemplateViewModel(Core.Models.ReportTemplate template)
        {
            _template = template;
        ...
        public Core.Models.ReportType Type => _template.Type;
        ...
        private string GetTypeDisplayName(Core.Models.ReportType type)
        {
            return type switch
            {
                Core.Models.ReportType.DailySummary => "Ежедневная сводка",
                Core.Models.ReportType.CallsignActivity => "Активность позывного",
                Core.Models.ReportType.AreaAnalysis => "Анализ зоны",
                Core.Models.ReportType.CommunicationFlow => "Поток коммуникаций",
                Core.Models.ReportType.AlertSummary => "Сводка алертов",
                Core.Models.ReportType.PatternAnalysis => "Анализ паттернов",
                Core.Models.ReportType.Custom => "Пользовательский",
                _ => "Неизвестный"
            };
        }
    ...
    {
        private readonly Core.Models.ReportParameter _parameter;
        private object _value;
        private bool _hasError;
        private string _errorMessage;

        public ReportParameterViewModel(Core.Models.ReportParameter parameter)
        {
    ...
    {
        private readonly Core.Models.GeneratedReport _report;
        private bool _isSelected;

        public GeneratedReportViewModel(Core.Models.GeneratedReport report)
        {
            _report = report;
        }
        ...
        public Core.Models.ReportStatus Status => _report.Status;

        public string StatusDisplay => GetStatusDisplayName(_report.Status);

        public string StatusColor => GetStatusColor(_report.Status);

        public bool IsReady => _report.Status == Core.Models.ReportStatus.Completed;

        public bool IsPending => _report.Status == Core.Models.ReportStatus.Pending;

        public bool IsGenerating => _report.Status == Core.Models.ReportStatus.Generating;

        public bool IsFailed => _report.Status == Core.Models.ReportStatus.Failed;
        ...
        private string GetStatusDisplayName(Core.Models.ReportStatus status)
        {
            return status switch
            {
                Core.Models.ReportStatus.Pending => "В очереди",
                Core.Models.ReportStatus.Generating => "Генерируется",
                Core.Models.ReportStatus.Completed => "Готов",
                Core.Models.ReportStatus.Failed => "Ошибка",
                _ => "Неизвестно"
            };
        }

        private string GetStatusColor(Core.Models.ReportStatus status)
        {
            return status switch
            {
                Core.Models.ReportStatus.Pending => "#FFA500", // Оранжевый
                Core.Models.ReportStatus.Generating => "#1E90FF", // Синий
                Core.Models.ReportStatus.Completed => "#32CD32", // Зеленый
                Core.Models.ReportStatus.Failed => "#FF4500", // Красный
                _ => "#808080" // Серый
            };
        }
    ...
    public class GenerateReportViewModel : INotifyPropertyChanged
    {
        ...
        public bool IsDailySummary => SelectedTemplate?.Type == Core.Models.ReportType.DailySummary;
        public bool IsCallsignReport => SelectedTemplate?.Type == Core.Models.ReportType.CallsignActivity;
        public bool IsAreaReport => SelectedTemplate?.Type == Core.Models.ReportType.AreaAnalysis;
        public bool IsCommunicationFlow => SelectedTemplate?.Type == Core.Models.ReportType.CommunicationFlow;
        public bool IsAlertSummary => SelectedTemplate?.Type == Core.Models.ReportType.AlertSummary;
    ...
    public class ReportStatisticsViewModel : INotifyPropertyChanged
    {
        ...
        public ReportStatisticsViewModel(Services.IReportService reportService)
        {
            _reportService = reportService;
    ...
    public class DailySummaryViewModel : INotifyPropertyChanged
    {
        ...
        private bool _isLoading;
        private Core.Models.DailySummaryReport _reportData;

        public DailySummaryViewModel(Services.IReportService reportService)
        {
            _reportService = reportService;
        }
        ...
        public Core.Models.DailySummaryReport ReportData
        {
            get => _reportData;
            set
            {
        ...
        public List<Core.Models.CallsignActivity> TopCallsigns => ReportData?.TopCallsigns ?? new();

        public List<Core.Models.AreaActivity> TopAreas => ReportData?.TopAreas ?? new();

        public List<Core.Models.AlertSummary> Alerts => ReportData?.Alerts ?? new();

        public List<Core.Models.PatternSummary> DetectedPatterns => ReportData?.DetectedPatterns ?? new();

        public List<Core.Models.KeyObservation> Observations => ReportData?.Observations ?? new();
    ...
    public class CallsignDossierViewModel : INotifyPropertyChanged
    {
        ...
        private DateTime _endDate = DateTime.Today;
        private bool _isLoading;
        private Core.Models.CallsignDossier _dossierData;

        public CallsignDossierViewModel(Services.IReportService reportService)
        {
            _reportService = reportService;
        }
        ...
        public Core.Models.CallsignDossier DossierData
        {
            get => _dossierData;
            set
            {
    ...
    public class AreaAnalysisViewModel : INotifyPropertyChanged
    {
        ...
        private DateTime _endDate = DateTime.Today;
        private bool _isLoading;
        private Core.Models.AreaActivityReport _reportData;

        public AreaAnalysisViewModel(Services.IReportService reportService)
        {
            _reportService = reportService;
        }
        ...
        public Core.Models.AreaActivityReport ReportData
        {
            get => _reportData;
            set
            {
