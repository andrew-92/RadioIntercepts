...
namespace RadioIntercepts.WpfApp.Services
{
    public interface IReportService
    {
        ...
        Task<List<ReportTemplate>> GetReportTemplatesAsync(bool onlyActive = true);
        Task<ReportTemplate> GetReportTemplateAsync(int id);
        ...
        Task ToggleTemplateAsync(int id, bool isActive);

        // Генерация отчетов
        Task<GeneratedReport> GenerateReportAsync(int templateId, Dictionary<string, object> parameters);
        Task<GeneratedReport> GenerateReportAsync(string templateName, Dictionary<string, object> parameters);

        // Специализированные отчеты
        Task<GeneratedReport> GenerateDailySummaryAsync(DateTime date, ReportFormat format = ReportFormat.Pdf);
        Task<GeneratedReport> GenerateCallsignDossierAsync(string callsign, DateTime startDate, DateTime endDate, ReportFormat format = ReportFormat.Pdf);
        Task<GeneratedReport> GenerateAreaActivityReportAsync(string area, DateTime startDate, DateTime endDate, ReportFormat format = ReportFormat.Pdf);

        // Управление сгенерированными отчетами
        Task<List<GeneratedReport>> GetGeneratedReportsAsync(DateTime? from = null, DateTime? to = null, string? templateName = null);
        Task<GeneratedReport> GetGeneratedReportAsync(string reportId);
        Task DeleteGeneratedReportAsync(string reportId);
        Task<byte[]> DownloadReportContentAsync(string reportId);

        // Статус отчетов
        Task<ReportStatus> GetReportStatusAsync(string reportId);
        Task<List<GeneratedReport>> GetPendingReportsAsync();
        Task<List<GeneratedReport>> GetFailedReportsAsync();
        Task RetryFailedReportAsync(string reportId);

        // Данные для отчетов
        Task<DailySummaryReport> GetDailySummaryDataAsync(DateTime date);
        Task<CallsignDossier> GetCallsignDossierDataAsync(string callsign, DateTime startDate, DateTime endDate);
        Task<AreaActivityReport> GetAreaActivityDataAsync(string area, DateTime startDate, DateTime endDate);

        // Статистика отчетов
        Task<ReportStatistics> GetReportStatisticsAsync(DateTime? startDate = null, DateTime? endDate = null);
    }
    ...
    public class ReportService : IReportService
    {
        ...
        private ActivityTimeline BuildActivityTimeline(List<Message> messages, DateTime startDate, DateTime endDate)
        {
            ...
            timeline.AverageActivity = messages.Count > 0
                ? (double)messages.Count / (endDate - startDate).TotalHours
                : 0;
